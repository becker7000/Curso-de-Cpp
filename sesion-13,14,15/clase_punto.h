...
class Punto{
    ...
    public:
        ...
        float pendiente(Punto punto_ent); // Ahora regresa un flotante
        void ecuacionConica(Punto punto_ent);
...
float Punto::pendiente(Punto punto_ent){
    float pendiente;
    pendiente=(this->y-punto_ent.getY())/(this->x-punto_ent.getX());
    return pendiente;
}

void Punto::ecuacionConica(Punto punto_ent){
    float m = this->pendiente(punto_ent);
    // b = y-mx
    // P(2,2), P(4,4) m=1 y=x
    float b = (this->y)-(m*this->x);
    cout<<"\n\t La ecuacion conica es: y = "<<m<<"x + "<<b;
}
